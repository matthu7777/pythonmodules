...
""" Adapted / bastardised from TRM's code by MJG to use q and deltai rather than q and i directly. 
	Changes: Lcurve.__init__() and Lcurve.set() should now both take deltai and use it to calculate iangle. 
	I believe calculations of chisq, lnprob etc should be unaffected
"""
            ...
            for line in fp:
                ...
                if line[0:2] == '##':
                    ...
                    if method != self.method:
                        raise MCMCError('Chain.add_file: method of file ' + fname +
                                        ' differs from that already loaded')
...
class Lcurve(subs.Odict):
    ...
    def ok(self):
        ...
        #self.vcheck('iangle', 0., 90.) and
        return self.vcheck('q', 0.001, 100.) and \
    ...
    def write(self,fname=None):
        ...
        for key,value in self.iteritems():
			## Here we fix the model so that it uses iangle (MJG)
            if key == "deltai":
                key = "iangle"
                q = float(self["q"][0])
                value = np.copy(value)
                value[0] = findi(q, float(value[0]))
            fptr.write('%-20s =' % key)
            for item in value:
                fptr.write(' ' + str(item))
            fptr.write('\n')
