...
can then be treated with FFTs. This can be fast for full frequency searches,
but it can be also rather expensive in terms of memory.
...
def __spread__(y, yy, x, m):
  """
  Given an array yy, extirpolate (spread) a value y into
  ...
  Arguments::
    y : single value that will be added to yy
    yy : array to cast y into
    x : x pixel value (0 to len(yy)-1) equivalent to y, can be non-integer
    m : order of extirpolation

  yy is modified on exit.
  """
...
#    ilo = min( max(ilo, 1), n-m+1)
    ...
    for j in range(ihi-1,ilo-1,-1):
      nden  *=(j-ihi)/float(j+1-ilo)
      yy[j] += y*fac/(nden*(x-j))

def fls(x,y,ofac,hifac, MACC=4):
  """ function fls
    Given abscissas x (which need not be equally spaced) and ordinate
  ...
  for j in range(0L, n):
    __spread__(y[j]-ave,wk1,ck[j],MACC)
    __spread__(1.0,wk2,ckk[j],MACC)
...
def fwls( x, y, e, ofac, hifac, MACC=4):
  ...
  for j in range(0L, n):
    __spread__(w[j]*(y[j]-wave),wk1,ck[j],MACC)
    __spread__(w[j],wk2,ckk[j],MACC)
...
def fwmls( x, y, e, ofac, hifac, amp=False, MACC=4):
  ...
  # "extirpolate" the data into the workspaces a trick to carry out double
  # frequency FFTs is used.
  wk1 = np.zeros(ndim, dtype='complex')
  wk2 = np.zeros(ndim, dtype='complex')
  ...
  for j in range(0L, n):
    __spread__(w[j]*(y[j]-wave),wk1,ck[j],MACC)
    __spread__(w[j],wk2,ck[j],MACC)
    __spread__(w[j],wk3,ckk[j],MACC)
  ...
  # now have sums of weight*cos()*y etc.  wc2 = sum weight*cos(2*k*x), etc,
  # allowing computation of sum weight*cos()**2 etc, e.g.
  #
  # wcc = (wsum + wc2)/2.
  # wss = (wsum - wc2)/2.
  # wcs = ws2/2.
  ...
  # "Normal equation" approach followed here where we think of linear
  # least-squares fit of y = c + a*cos() + b*sin() End up with 3x3 matrix A of
  # form:
  #
  # A = w   wc  ws
  #     wc  wcc wcs
  ...
  #     wsy
  #
  # (all cases summed over the data points). In fact we have one of these for
  # each frequency. The FFTs above have given us the values of the sums
  ...
  # subtraction of wave = wy / w above, ensures that the wy (first element in
  # the b vector) at this point = 0, saving some computation.
...
def wmls( x, y, e, f, amp=False, nh=1):
  ...
  for n,freq in enumerate(f):
    ...
    bt = b.transpose()
    if amp:
      p[n] = np.sqrt((AI[1,1]*b[1]+AI[1,2]*b[2])**2 +
                     (AI[2,1]*b[1]+AI[2,2]*b[2])**2)
    else:
      p[n] = float(bt*AI*b)/2.
