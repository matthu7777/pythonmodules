...
def ring2(rings, r1, x1, y1, r2, x2, y2):
    ...
    if both.any():
        ...
        for l1,h1,l2,h2 in zip(lo1[both],hi1[both],lo2[both],hi2[both]):
            ...
            # 7 separate cases. I wonder if this is all of them or
            # whether it can be shortened?
            if l1 <= l2 and h1 >= h2:
                # if range 1 covers range 2, only range 2 is visible
                vpart[n] = h2-l2
                ...
                # range 2 overlaps range 1. Only overlap
                # is visible. NB we know that h2 > h1
                # here because otherwise test 1 would have
                # worked
                vpart[n] = h1-l2
                if h2 >= l1+1:
                    vpart[n] += h2-l1-1
                ...
                # overlapping range1 on its (range1's) next cycle
                vpart[n] = h2-l1-1
...
def flux3(r1, r2, r3, limb1, limb2, limb3, n1, n2, n3, p1, p2, p3):
    ...
    if not hasattr(flux3,'rings2') or r2 != flux3.r2 or \
    ...
    if not hasattr(flux3,'rings3') or r3 != flux3.r3 or \
    ...
    if p1.z <= p2.z and p1.z <= p3.z:
        ...
        full3, total3, vis3 = cring2(flux3.rings3,
                                     r1, p1.x-p3.x, p1.y-p3.y,
                                     r2, p2.x-p3.x, p2.y-p3.y)
        if p1.z < p2.z:
...
if __name__ == '__main__':
    ...
    r1 = 0.66 # radius star 1
    limb1 = Limb(Limb.POLY, 0.3)
    n1 = 100
    a1 = 106. # semi-major axis of its orbit around 1+2+3 CoM

    s2 = 1.0 # surface brightness star 2
    r2 = 0.24 # radius star 2
    limb2 = Limb(Limb.POLY, 0.3)
    n2 = 50
    a2 = 1.2 # semi-major axis of its orbit around 2+3 CoM

    s3 = 0.8 # surface brightness star 3
    r3 = 0.22 # radius star 3
    limb3 = Limb(Limb.POLY, 0.3)
    n3 = 50
    a3 = 1.4 # semi-major axis of its orbit around 2+3 CoM

    period1 = 205. # period of 1 + (2+3) orbit
    t1 = 0. # zero time of 1 + (2+3) orbit
    iangle1 = 89.99 # orbital inclination of 1 + (2+3) orbit

    a23 = 152. # semi-major axis of 2+3's orbit about 1+2+3
