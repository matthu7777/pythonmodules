...
class Axis(object):
    ...
    def __imul__(self, other):
        ...
        if isinstance(other, Axis):
            ...
            if isinstance(other, np.ndarray):
                self.data[ok] *= other[ok]
                if self.has_errors:
                    self.errors[ok] *= np.abs(other[ok])
            else:
                self.data[ok] *= other
                if self.has_errors:
    ...
    def log10(self):
        ...
        if self.has_errors:
            self.errors[ok]  = self.errors[ok]/self.data[ok]/math.log(10.)
    ...
    def mean(self):
        """
        Returns the mean, ignoring masked points.
        """
        if self._good is None:
            return self.data.mean()
        else:
            return self.data[self.good].mean()
    ...
    def blkavg(self, nblock):
        """
        Averages the Axis in integer blocks of nblock at a time.
        Masked points propagate. i.e. a block that includes any bad block
        will be flagged as bad. Any odd elements at the end of the arrays
        will be lost. i.e. given a 5 element input array and nblock=2,
        the output would have 2 elements generated from elements 0,1,2,3
        of the input arrays.

        Arguments::

        nblock : integer
              Size of blocks. Must be > 1 for any effect. < 1 raises a DnlError

        """
        if nblock > 1:
            nnew = nblock*(len(self) // nblock)
            if nnew < len(self):
                """
                Chop dangling elements
                """
                self.data = self.data[:nnew]
                if self.has_errors:
                    self.errors = self.errors[:nnew]
                if self.has_mask:
                    self.good = self.good[:nnew]

            self.data = np.mean(self.data.reshape(-1,nblock),1)

            if self.has_errors:
                self.errors = np.sqrt(np.mean((self.errors**2).reshape(-1,nblock),1)/nblock)

            if self.has_mask:
                # Haven't thought of a way to avoid a loop here.
                # Basically this next bit means that an output pixel is only
                # marked OK if all contributing input pixels are.
                new_mask = np.empty((nnew//nblock,),bool)
                for n in xrange(len(new_mask)):
                    new_mask[n] = self.good[nblock*n:nblock*(n+1)].all()
                self.good = new_mask

        elif nblock < 1:
            raise DnlError('blkavg: nblock must be > 1')

    def repeat(self, nblock):
        """
        Semi-inverse of blkavg. Each element is repeated nblock times. Of course at the end of
        applying this routine, the uncertainties become a bit meaningless since the data is 100%
        correlated in each repeat block.

        Arguments::

        nblock : integer
              Number of times to expand. Must be > 1 for any effect.
        """

        self.data = np.repeat(self.data, nblock)

        if self.has_errors:
            self.errors = np.repeat(self.errors, nblock)

        if self.has_mask:
            self.good = np.repeat(self.good, nblock)

    def clip(self, nleft, nright):
        """
        Clips elements from the ends of the arrays

        Arguments::

          nleft: integer
               number to remove from the left (start) of the arrays

          nright: integer
               number to remove from the right (end) of the arrays
        """
        if nleft < 0 or nright < 0:
            raise DnlError('clip: nleft and nright cannot be negative')

        nright = len(self)-nright

        self.data = self.data[nleft:nright]
        if self.has_errors:
            self.errors = self.errors[nleft:nright]
        if self.has_mask:
            self.good = self.good[nleft:nright]
...
class Dset(object):
    ...
    are good. Note Dsets have no plotting methods. These are devolved to derived
    classes, e.g. MplDset for matplotlib-enabled plots.
    ...
    def good(self, ngood):
        ...
        if ngood is None:
            self._good = None
        elif isinstance(ngood, np.ndarray) and ngood.ndim == 1:
            if len(ngood) == len(self):
                self._good = copy.copy(ngood)
            else:
                raise DnlError('length of ngood array [' +
                               str(len(ngood)) + '] differs from data [' +
                               str(len(self)) + ']')
        elif isinstance(ngood, (list, tuple)):
            self._good = np.asarray(ngood, dtype=np.bool)
            if self._good.ndim != 1:
                raise DnlError('ngood converts to a numpy array but is not 1D')
        else:
            raise DnlError('ngood is not a 1D numpy.ndarray')
    ...
    def has_mask(self):
        """
        Whether there is a mask array (in addition to mask arrays on each Axis
        """
        return self._good is not None
    ...
    def blkavg(self, nblock):
        """
        Reduces a Dset by averaging each axis in blocks of nblock pixels.
        Propagates mask.
        """
        self.x.blkavg(nblock)
        self.y.blkavg(nblock)

        if nblock > 1 and self.has_mask:
            nnew = nblock*(len(self._good) // nblock)
            if nnew < len(self._good):
                # Chop dangling elements
                self._good = self._good[:nnew]

            new_mask = np.empty((nnew//nblock,),bool)
            for n in xrange(len(new_mask)):
                new_mask[n] = self._good[nblock*n:nblock*(n+1)].all()
            self._good = new_mask

    def repeat(self, nblock):
        """
        Expands a Dset by repeating each element nblock times.
        """
        self.x.repeat(nblock)
        self.y.repeat(nblock)
        if self.has_mask:
            self._good = np.repeat(self._good, nblock)
    ...
    def clip(self, nleft, nright):
        ...
        self.x.clip(nleft, nright)
        self.y.clip(nleft, nright)
        if self.has_mask:
            nright = len(self._good)-nright
            self._good = self._good[nleft:nright]

    def setpos(self, target, ra, dec, system='ICRS', pmra=None, pmdec=None,
        ...
        self.head['TARGET'] = (target,'Target name')
        ...
        if parallax is not None:
            self.head['PARALLAX'] = (parallax,'Parallax to target in arcsec')
    ...
    def getpos(self):
        ...
        Returns: (target, ra, dec, system, pmra, pmdec, epoch, parallax, rv)
        where::
        ...
        return (target, ra, dec, system, pmra, pmdec, epoch, parallax, rv)
    ...
    def mjd2bmjd(self):
        ...
        # get positional data
        target, ra, dec, system, pmra, pmdec, epoch, parallax, rv = \
            self.getpos()
    ...
    def mjd2hmjd(self):
        """
        Convert X-axis times from MJD (UTC) to HMJD (UTC).
        Target and telescope data must have been set. No checks
        are made for valid MJDs.
        """
        ...
        # update
        self.x.data[self.x.good] = hutcs
        self.x.label = 'HMJD (UTC)'
        self.x.units = 'days'

    def phase(self, T0, P, label='Orbital phase'):
        """
        Convert X-axis times into phases.
        """
        self.x -= T0
        self.x /= P
        self.x.label = label
    ...
    def amass(self):
        ...
        # Run checks
        telescope, observatory, longitude, latitude, height = self.gettel()
        target, ra, dec, system, pmra, pmdec, epoch, parallax, rv = \
            self.getpos()
        ...
        # all tests passed
        for i in xrange(len(self.x.data)):
            airmass,alt,az,ha,pa,dz = \
                sla.amass(self.x.data[i],
                          longitude, latitude, height, ra, dec,
                          0.55, pmra, pmdec, epoch, parallax, rv)
    ...
    def fnu2flam(self, xu=1.e-10, yu=1.):
        """
        Convert y-axis from a flux density per unit frequency (fnu)
        to one per unit wavelength (flambda). The x-axis is assumed
        to be a wavelength.

        Arguments::

          xu : (float)
             wavelength units in metres (1.e-10 = Angstroms)

          yu : (float)
             fnu units in mJy (1 mJy = 1.e-26 ergs/s/cm^2/Hz)

        The output units are ergs/s/cm^2/A
        """
        fac = yu*(1.e-36*subs.C/xu**2)/self.x.data**2
        self.y *= fac

    def flam2fnu(self, xu=1.e-10, yu=1.):
        """
        Convert y-axis from a flux density per unit wavelength (flambda)
        to one per unit frequency (fnu). The x-axis is assumed
        to be a wavelength.
        ...
        Arguments::
          ...
          yu : (float)
             flam units in ergs/s/cm^2/A 

        The output units are mJy
        """
        fac = yu*self.x.data**2/(1.e-36*subs.C/xu**2)
        self.y *= fac
