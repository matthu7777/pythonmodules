...
lrochep = os.path.join(os.environ['TRM_SOFTWARE'], 'bin', 'lcurve', 'lroche')
lroches = os.path.join(os.environ['TRM_SOFTWARE'], 'bin', 'lcurve', 'lroches')

if not os.path.exists(lrochep):
    print 'Could not find',lrochep
    exit(1)

if not os.path.exists(lroches):
    print 'Could not find',lroches
    exit(1)
...
def _chain_from_file(fname):
    ...
    Returns (fobj,names,model,nstore,nwalker,stretch,chmin,jump)
    ...
    try:
        ...
        jump = None
    return (fobj,names,model,nstore,nwalker,stretch,chmin,jump)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) == 1 or len(args) == 2 or len(args) == 3:
            ...
            fobj,self.names,self.model,self.nstore,self.nwalker,self.stretch,\
                self.chmin,self.jump = _chain_from_file(args[0])
            ...
            if 'fast' in kwargs and not kwargs['fast']:
                # Read the data
                data = []
                ns = 0
                for line in fobj:
                ...
                if nskip >= 0:
                    data = data[(nthin+1)*nskip*self.nwalker::nthin+1]
                else:
                    data = data[-(nthin+1)*nskip*self.nwalker::nthin+1]
                ns = len(data)
            ...
            if args[2] is not None:
                self.jump    = copy.deepcopy(args[2])
            else:
                self.jump    = None
            self.nwalker = int(args[3])
            self.stretch = float(args[4])
            if self.jump is not None:
                self.names   = self.jump.par + list(args[5])
            else:
                self.names   = list(args[5])
            self.chmin   = float(args[6])
            self.vals    = None
            self.nmods   = 0
        else:
            raise MCMCError('Chain.__init__: could not interpret arguments of Chain constructor; expecting 1, 2, 3 or 7')

    def add_file(self, fname, nskip=0, nthin=0):
        """
        Adds in data from another file

         fname -- name of file to add in
         nskip -- number of point groups to skip (-ve to keep the final nskip groups)
         nthin -- number to skip over per point accepted
        """
        ...
        with open(fname) as fp:
            ...
            for line in fp:
                ...
                if line[0:2] == '##':
                    ...
                    names = [n.strip() for n in names]
                    if names != self.names:
                        raise MCMCError('Chain.add_file: file ' + fname +
                                        ' contains a different set of parameters or parameters'
                                        + ' in a different order from those already read.')
                elif line.startswith('# nstore = '):
                    nstore = int(line[line.find('=')+1:].strip())
                    if nstore != self.nstore:
                        raise MCMCError('Chain.add_file: nstore of file ' + fname +
                                        ' differs from that already loaded')

                elif line.startswith('# nwalker = '):
                    nwalker = int(line[line.find('=')+1:].strip())
                    if nwalker != self.nwalker:
                        raise MCMCError('Chain.add_file: nwalker of file ' + fname +
                                        ' differs from that already loaded')

                elif line.startswith('# stretch = '):
                    stretch = float(line[line.find('=')+1:].strip())
                    if stretch != self.stretch:
                        raise MCMCError('Chain.add_file: stretch of file ' + fname +
                                        ' differs from that already loaded')
            ...
            tjump = Jump(fp)
            if tjump != self.jump:
                print 'Jump distribution of file ' + fname + \
                    ' differs from that already loaded, but will ignore.'
#                raise MCMCError('Chain.add_file: jump distribution of file ' + fname +
#                                ' differs from that already loaded')
    ...
    def pmax(self):
        """
        Returns maximum -2*ln(posterior prob) stored in the chain.
        Returns 1e30 if there are no values yet.
        ...
        if 'pprob' in self.names:
            ...
            if self.vals is not None:
                ...
                return -1.e30
        elif 'lp' in self.names:
            if self.vals is not None:
                nc = self.names.index('lp')
                return self.vals[:,nc].max()
            else:
                return -1.e30
        else:
            raise MCMCError('Chain.pmin: no pprob, lpost or lp found')
...
class Fchain(Chain):
    ...
    def __init__(self, *args):
        ...
        if len(args) == 1:
            ...
            Chain.__init__(self, args[0], args[1], args[2], args[3], args[4], args[5], args[6])
            fname = args[7]
            self.tofile(fname)
        else:
...
class IChain(object):
    ...
    def __init__(self, fname):
        ...
        self.fobj,self.names,self.model,self.nstore,self.nwalker,self.stretch,\
            self.chmin,self.jump = _chain_from_file(fname)
        self.nline = 0
...
class Lcurve(subs.Odict):
    ...
    def chisqlp(self, data, serial):
        ...
        serial == True implies it will run the serial version of lroche
                  rather than the parallelised one.
        """
        ...
        args = (lroches if serial else lrochep, 'model=' + tfile, 'data=' + data, 'noise=0',
                'scale=yes', 'seed=12345', 'nfile=0', 'device=null', 'nodefs')
        ...
        if len(subout) == 1:
            ...
            chisq = wnok = wdwarf = None
            print 'Output from lroche failure: ' + sout + '\n' + serr + '\n'
    ...
    def trial(self, trl, data, cold, lpold, cmin, tmodel, wdold, plarge=0., slarge=3.):
        ...
        if self.ok():
            ...
            if tmodel:
                self.write(tmodel)
            cnew,wnok,wdwarf = self.chisqlp(data)
...
class Jump(object):
    ...
    def trial(self, pd):
        ...
        """
        td = dict(pd)
        delta = self.sig*np.random.multivariate_normal(np.zeros_like(self.sig),
                                                       self.corr)
